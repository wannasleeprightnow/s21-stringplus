#include <check.h>
#include <limits.h>
#include <stdarg.h>
#include <stdio.h>

#include "../../../s21_string.h"
#include "../../tests.h"

void test_handle_u(char *buffer, int *j, int spec, Flags *flags, ...) {
  va_list args;
  va_start(args, flags);
  handle_u(buffer, j, &spec, &args, flags);
  va_end(args);
}

START_TEST(test_basic_unsigned) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.accuracy = -1};
  unsigned value = 12345;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "12345");
}
END_TEST

START_TEST(test_zero) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.accuracy = -1};
  unsigned value = 0;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "0");
}
END_TEST

START_TEST(test_max_unsigned) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.accuracy = -1};
  unsigned value = UINT_MAX;
  test_handle_u(buffer, &j, 0, &flags, value);
  char expected[20];
  sprintf(expected, "%u", UINT_MAX);
  ck_assert_str_eq(buffer, expected);
}
END_TEST

START_TEST(test_width) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.width = 10, .accuracy = -1};
  unsigned value = 12345;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "     12345");
}
END_TEST

START_TEST(test_zero_padding) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.width = 10, .with_zeros = 1, .accuracy = -1};
  unsigned value = 12345;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "0000012345");
}
END_TEST

START_TEST(test_precision) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.accuracy = 5};
  unsigned value = 123;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "00123");
}
END_TEST

START_TEST(test_zero_precision) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.accuracy = 0};
  unsigned value = 0;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "");
}
END_TEST

START_TEST(test_long_unsigned) {
  char buffer[30] = {0};
  int j = 0;
  Flags flags = {.is_long = 'l', .accuracy = -1};
  unsigned long value = ULONG_MAX;
  test_handle_u(buffer, &j, 0, &flags, value);
  char expected[30];
  sprintf(expected, "%lu", ULONG_MAX);
  ck_assert_str_eq(buffer, expected);
}
END_TEST

START_TEST(test_short_unsigned) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.accuracy = -1};
  unsigned short value = 12345;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "12345");
}
END_TEST

START_TEST(test_combined_flags) {
  char buffer[20] = {0};
  int j = 0;
  Flags flags = {.width = 10, .accuracy = 5, .with_zeros = 1};
  unsigned value = 123;
  test_handle_u(buffer, &j, 0, &flags, value);
  ck_assert_str_eq(buffer, "     00123");
}
END_TEST

Suite *handle_u_suite(void) {
  Suite *s = suite_create("handle_u");
  TCase *tc = tcase_create("core");

  tcase_add_test(tc, test_basic_unsigned);
  tcase_add_test(tc, test_zero);
  tcase_add_test(tc, test_max_unsigned);
  tcase_add_test(tc, test_width);
  tcase_add_test(tc, test_zero_padding);
  tcase_add_test(tc, test_precision);
  tcase_add_test(tc, test_zero_precision);
  tcase_add_test(tc, test_long_unsigned);
  tcase_add_test(tc, test_short_unsigned);
  tcase_add_test(tc, test_combined_flags);

  suite_add_tcase(s, tc);
  return s;
}